...
class HoneyFramesNetCountField extends StatelessWidget {
  const HoneyFramesNetCountField({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final emptyNet = context.select<InspectionBloc, int?>((bloc) => 
    ...
    final oldValue = context.select<InspectionBloc, int?>((bloc) => 
      bloc.state.getOldFieldValue<int>('framesMoved.honeyNet')) ?? 0;
    ...
    int min = -totalFrames - math.min(0, emptyNet);
    int max = (maxFrames - totalFrames + boxNetChange * framesPerBox - emptyNet);
    ...
    if (value != clampedValue && fieldState != FieldState.old) {
      ...
      context.read<InspectionBloc>().add(
        UpdateFieldEvent('framesMoved.honeyNet', clampedValue),
      );
    }
    ...
    return NetCountInputField(
      ...
      fieldName: 'framesMoved.honeyNet',
      ...
      onChanged: (newValue) {
        ...
        context.read<InspectionBloc>().add(
          UpdateFieldEvent('framesMoved.honeyNet', newValue),
        );
      },
      onReset: () => context.read<InspectionBloc>().add(
        const ResetFieldEvent('framesMoved.honeyNet'),
      ),
    );
  }
}
