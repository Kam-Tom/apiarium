...
class HoneyBroodFramesNetField extends StatelessWidget {
  const HoneyBroodFramesNetField({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final emptyBroodNet = context.select<InspectionBloc, int?>((bloc) => 
    ...
    final oldValue = context.select<InspectionBloc, int?>((bloc) => 
      bloc.state.getOldFieldValue<int>('framesMoved.broodNet')) ?? 0;
    ...
    int min = -totalBroodFrames - math.min(0,emptyBroodNet);
    int max = (maxBroodFrames - totalBroodFrames + broodBoxNetChange * framesPerBroodBox - emptyBroodNet);
    ...
    if(value != clampedValue && fieldState != FieldState.old) {
      // Only update if not setting to 0 (handled by bloc now)
      context.read<InspectionBloc>().add(
        UpdateFieldEvent('framesMoved.broodNet', clampedValue),
      );
    }
    ...
    return NetCountInputField(
      ...
      fieldName: 'framesMoved.broodNet',
      ...
      onChanged: (newValue) {
        ...
        context.read<InspectionBloc>().add(
          UpdateFieldEvent('framesMoved.broodNet', newValue),
        );
      },
      onReset: () => context.read<InspectionBloc>().add(
        const ResetFieldEvent('framesMoved.broodNet'),
      ),
    );
  }
}
