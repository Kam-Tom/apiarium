...
import 'package:apiarium/shared/widgets/modals/history_log/history_log_modal.dart';
import 'package:apiarium/core/router/app_router.dart';
import 'package:apiarium/core/di/dependency_injection.dart';
import 'package:go_router/go_router.dart';
import 'cubit/queen_detail_cubit.dart';
import 'widgets/queen_info_section.dart';
import 'widgets/inspection_tile.dart';

class QueenDetailPage extends StatelessWidget {
  final String queenId;

  const QueenDetailPage({super.key, required this.queenId});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => QueenDetailCubit(
        queenService: getIt<QueenService>(),
        historyService: getIt<HistoryService>(),
        inspectionService: getIt<InspectionService>(),
      )..loadQueenDetail(queenId),
      child: const _QueenDetailView(),
    );
  }
}

class _QueenDetailView extends StatelessWidget {
  const _QueenDetailView();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: BlocBuilder<QueenDetailCubit, QueenDetailState>(
          builder: (context, state) {
            return Text(
              state.queen?.name ?? 'details.queen.title'.tr(),
      ...
      body: BlocConsumer<QueenDetailCubit, QueenDetailState>(
        listener: (context, state) {
          if (state.status == QueenDetailStatus.error) {
        ...
        builder: (context, state) {
          return switch (state.status) {
            QueenDetailStatus.initial || QueenDetailStatus.loading => 
              const Center(child: CircularProgressIndicator(color: Colors.amber)),
            QueenDetailStatus.error => _ErrorView(
              message: state.errorMessage ?? 'common.error_occurred'.tr(),
              onRetry: () => context.read<QueenDetailCubit>().refresh(
                (context.read<QueenDetailCubit>().state.queen?.id ?? '') as String,
              ),
            ),
            QueenDetailStatus.loaded => _LoadedView(
              queen: state.queen!,
              queenBreed: state.queenBreed,
...
class _LoadedView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      onRefresh: () => context.read<QueenDetailCubit>().refresh(queen.id),
      color: Colors.amber,
          ...
          children: [
            Container(
              ...
              child: TabBar(
                ...
                tabs: [
                  Tab(text: 'details.queen.basicInfo'.tr()),
                  Tab(text: 'details.queen.history'.tr()),
                  Tab(text: 'details.queen.inspections'.tr()),
                ],
              ),
            ),
            Expanded(
              child: TabBarView(
                children: [
                  QueenInfoSection(queen: queen, queenBreed: queenBreed),
        ...
        child: Center(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text(
                'details.queen.no_inspections'.tr(),
                style: TextStyle(fontSize: 16, color: Colors.grey.shade600),
              ),
            ],
          ),
  ...
  void _showInspectionDetails(BuildContext context, Inspection inspection) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text('details.queen.inspection_details'.tr()),
        content: SingleChildScrollView(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('${inspection.hiveName} - ${inspection.apiaryName ?? 'details.queen.unknown_apiary'.tr()}'),
              ...
              if (inspection.data != null && inspection.data!.isNotEmpty) ...[
                const SizedBox(height: 16),
                Text('details.queen.inspection_data'.tr()),
