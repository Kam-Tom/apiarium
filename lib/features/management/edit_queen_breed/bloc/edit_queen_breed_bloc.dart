...
class EditQueenBreedBloc extends Bloc<EditQueenBreedEvent, EditQueenBreedState> {
  ...
  EditQueenBreedBloc({
    ...
    on<EditQueenBreedSpringDevelopmentRatingChanged>(_onSpringDevelopmentRatingChanged);
    on<EditQueenBreedGentlenessRatingChanged>(_onGentlenessRatingChanged);
    on<EditQueenBreedSwarmingTendencyRatingChanged>(_onSwarmingTendencyRatingChanged);
    on<EditQueenBreedHeatToleranceRatingChanged>(_onHeatToleranceRatingChanged);
    ...
    try {
      ...
      if (event.breedId != null) {
        ...
        emit(state.copyWith(
          ...
          springDevelopmentRating: breed.springDevelopmentRating ?? 0,
          gentlenessRating: breed.gentlenessRating ?? 0,
          swarmingTendencyRating: breed.swarmingTendencyRating ?? 0,
          winterHardinessRating: breed.winterHardinessRating ?? 0,
          diseaseResistanceRating: breed.diseaseResistanceRating ?? 0,
          heatToleranceRating: breed.heatToleranceRating ?? 0,
  ...
  void _onSpringDevelopmentRatingChanged(EditQueenBreedSpringDevelopmentRatingChanged event, Emitter<EditQueenBreedState> emit) {
    emit(state.copyWith(springDevelopmentRating: event.rating));
  }
  void _onGentlenessRatingChanged(EditQueenBreedGentlenessRatingChanged event, Emitter<EditQueenBreedState> emit) {
    emit(state.copyWith(gentlenessRating: event.rating));
  }
  void _onSwarmingTendencyRatingChanged(EditQueenBreedSwarmingTendencyRatingChanged event, Emitter<EditQueenBreedState> emit) {
    emit(state.copyWith(swarmingTendencyRating: event.rating));
  }
  ...
  void _onHeatToleranceRatingChanged(EditQueenBreedHeatToleranceRatingChanged event, Emitter<EditQueenBreedState> emit) {
    emit(state.copyWith(heatToleranceRating: event.rating));
  }
    ...
    try {
      if (state.isCreating) {
        await _queenService.createQueenBreed(
          ...
          springDevelopmentRating: state.springDevelopmentRating,
          gentlenessRating: state.gentlenessRating,
          swarmingTendencyRating: state.swarmingTendencyRating,
          winterHardinessRating: state.winterHardinessRating,
          diseaseResistanceRating: state.diseaseResistanceRating,
          heatToleranceRating: state.heatToleranceRating,
