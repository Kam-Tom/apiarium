...
class HiveRepository {
  ...
  Future<List<Hive>> getDeletedHives() async {
    ...
    deletedHives.sort((a, b) => b.updatedAt.compareTo(a.updatedAt)); // Most recent first
    return deletedHives;
  }
  ...
  Future<void> saveHive(Hive hive) async {
    try {
      ...
      Logger.i('Saved hive: ${hive.name}', tag: _tag);
    } catch (e) {
  ...
  Future<void> saveHivesBatch(List<Hive> hives) async {
    try {
      ...
      Logger.i('Saved ${hives.length} hives in batch', tag: _tag);
    } catch (e) {
  ...
  Future<void> syncFromFirestore(String userId, {DateTime? lastSyncTime}) async {
    try {
      ...
      Logger.i('Synced ${snapshot.docs.length} hives from Firestore', tag: _tag);
    } catch (e) {
